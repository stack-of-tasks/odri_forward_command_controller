  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  {
    pos_interface_names_.push_back(joint + "/" + hardware_interface::HW_IF_POSITION);
    vel_interface_names_.push_back(joint + "/" + hardware_interface::HW_IF_VELOCITY);
    eff_interface_names_.push_back(joint + "/" + hardware_interface::HW_IF_EFFORT);
...
OdriForwardCommandController::command_interface_configuration() const
{
  ...
  config.names.insert(config.names.end(), pos_interface_names_.begin(), pos_interface_names_.end());
  config.names.insert(config.names.end(), vel_interface_names_.begin(), vel_interface_names_.end());
  config.names.insert(config.names.end(), eff_interface_names_.begin(), eff_interface_names_.end());
  config.names.insert(config.names.end(), kp_interface_names_.begin(), kp_interface_names_.end());
  config.names.insert(config.names.end(), kd_interface_names_.begin(), kd_interface_names_.end());
  return config;
}
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  auto assign_interfaces =
    [&](const std::vector<std::string> & names, std::vector<LoanedRef> & out) -> bool {
    ...
    {
      RCLCPP_ERROR(
        get_node()->get_logger(), "Expected %zu interfaces for '%s', got %zu", n,
        names.empty() ? "?" : names[0].substr(names[0].rfind('/') + 1).c_str(), tmp.size());
      return false;
    }
    out = std::move(tmp);
    return true;
  ...
  if (
    !assign_interfaces(pos_interface_names_, pos_interfaces_) ||
...
controller_interface::CallbackReturn OdriForwardCommandController::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  auto apply = [&](std::size_t offset, std::vector<LoanedRef> & ifaces) {
    ...
    {
      if (std::isfinite(data[offset + i]) && !ifaces[i].get().set_value(data[offset + i]))
      {
        RCLCPP_WARN_THROTTLE(
          get_node()->get_logger(), *(get_node()->get_clock()), 1000,
          "Failed to set command interface '%s'", ifaces[i].get().get_name().c_str());
      }
    }
  };
